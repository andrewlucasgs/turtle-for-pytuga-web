...
def create_image(filename):
    '''Creates an image from the given filename'''
    return SVG.image(x=0, y=-29, width=36,
                     height=30, href=filename)
def create_circle(r):
...
class Screen(metaclass=Singleton):
    ...
    def _setup_canvas(self):
        ...
        self.svg_scene = SVG.svg(width=self.width, height=self.height, id="scene",
                                 preserveAspectRatio="xMidYMid meet",
                                 viewBox="0 0 {} {}".format(
                                     self.width, self.height))
        self.group_scene = SVG.g(id="group_scene")
        ...
        appendTo(self.group_scene, self.background_canvas)
        appendTo(self.group_scene, self.canvas)
        appendTo(self.group_scene, self.writing_canvas)
        appendTo(self.group_scene, self.turtle_canvas)
        appendTo(self.svg_scene, self.group_scene)
